...
filtered_players <- read.csv(paste(folder, "/data/filtered_players.csv", sep=""))
ftps_2023 <- read.csv(paste(folder, "/data/fantasypoints_2023.csv", sep = ""))
age <- read.csv(paste(folder, "/data/age_2023.csv", sep = ""))
yprr <- read.csv(paste(folder, "/data/yprr_2022.csv", sep = ""))
competition_change <- read.csv(paste(folder, "/data/competition_change2023.csv", sep = ""))
qb_skill <- read.csv(paste(folder, "/data/qb_skill2023.csv", sep = ""))
target_share <- read.csv(paste(folder, "/data/target_share2022.csv", sep = ""))
...
top12_actual <- actual_comp %>% filter(actual_rank <= 24)
...
comparison <- comparison %>%
  mutate(
    model_green  = (model_rank <= 24) & (abs(model_rank - actual_rank) <= 5),
    expert_green = (expert_rank <= 24) & (abs(expert_rank - actual_rank) <= 5)
  )
